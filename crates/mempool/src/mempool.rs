...
use crate::{
	...
	config::{
		BLOCK_GAS_LIMIT, LocalTransactionConfig, MIN_PROTOCOL_BASE_FEE, MempoolConfig, PoolConfig,
		TXPOOL_MAX_ACCOUNT_SLOTS_PER_SENDER,
	},
	error::{InvalidPoolTransactionError, PoolError, PoolErrorKind},
	identifier::{SenderId, TransactionId},
	mined::{AddedPendingTransaction, AddedTransaction, OnNewCanonicalStateOutcome},
	ordering::{FeeOrdering, TransactionOrdering},
	parked::{ParkedPool, QueuedOrd},
	pending::PendingPool,
	state::{SubPool, TxState},
	traits::{BlockInfo, PoolResult, PoolSize, PoolTransaction},
...
pub struct Mempool<T: TransactionOrdering> {
	...
	parked_pool: ParkedPool<QueuedOrd<T::Transaction>>,

	/// All transactions in the pool.
	all_transactions: AllTransactions<T::Transaction>,
}

impl<T: TransactionOrdering> Default for Mempool<T> {
	fn default() -> Self {
		Self {
...
impl<T: TransactionOrdering> Mempool<T> {
	...
	pub fn size(&self) -> PoolSize {
		PoolSize {
			...
			parked: self.parked_pool.len(),
			parked_size: self.parked_pool.size(),
	...
	pub fn set_block_info(&mut self, info: BlockInfo) {
		let BlockInfo { last_seen_block_hash, last_seen_block_number } = info;
	...
	pub(crate) fn parked_transactions(&self) -> Vec<Arc<ValidPoolTransaction<T::Transaction>>> {
		self.parked_pool.all().collect()
	}

	/// Returns an iterator over all transactions from parked pools
	pub(crate) fn parked_transactions_iter(
		&self,
	) -> impl Iterator<Item = Arc<ValidPoolTransaction<T::Transaction>>> + '_ {
		self.parked_pool.all()
	}

	/// Returns all parked transactions for the specified sender
	pub(crate) fn parked_txs_by_sender(
		&self,
		sender: SenderId,
	) -> Vec<Arc<ValidPoolTransaction<T::Transaction>>> {
		self.parked_transactions_iter().filter(|tx| tx.sender_id() == sender).collect()
	}
	...
	pub(crate) fn subpool_contains(&self, subpool: SubPool, id: &TransactionId) -> bool {
		match subpool {
			SubPool::Parked => self.parked_pool.contains(id),
	...
	fn remove_from_subpool(
		...
		let tx = match pool {
			SubPool::Parked => self.parked_pool.remove_transaction(tx),
	...
	fn add_transaction_to_subpool(
		...
		match pool {
			SubPool::Parked => self.parked_pool.add_transaction(tx),
			SubPool::Pending => {
	...
	pub(crate) fn discard_worst(&mut self) -> Vec<Arc<ValidPoolTransaction<T::Transaction>>> {
		...
		// 		parked_limit  => (parked_pool, queued_transactions_evicted),
		// 	]
		// );
	...
	pub fn assert_invariants(&self) {
		let size = self.size();
		let actual = size.pending + size.parked;
		assert_eq!(
			size.total, actual,
			"total size must be equal to the sum of all sub-pools, pending:{}, parked:{}",
			size.pending, size.parked
		);
		self.all_transactions.assert_invariants();
		self.pending_pool.assert_invariants();
		self.parked_pool.assert_invariants();
	}
}
...
impl<T: PoolTransaction> AllTransactions<T> {
	...
	fn set_block_info(&mut self, block_info: BlockInfo) {
		let BlockInfo { last_seen_block_hash, last_seen_block_number } = block_info;
	...
	pub(crate) fn insert_tx(
		...
		match self.txs.entry(*transaction.id()) {
			...
			Entry::Occupied(mut entry) => {
				...
				if existing_transaction.is_underpriced(maybe_replacement) {
...
impl SenderInfo {
	...
	const fn update(&mut self, state_nonce: u64, balance: u128) {
...
#[test]
fn test_insert_with_low_gas_price() {
	use crate::testutils::{MockTransaction, MockTransactionFactory};
	...
	let mut pool = AllTransactions::default();
	let mut tx = MockTransaction::default();
	tx.set_gas_price(MIN_PROTOCOL_BASE_FEE - 1);
	let valid_tx = f.validated(tx);

	let res = pool.insert_tx(valid_tx.clone(), on_chain_balance, on_chain_nonce);

	match res {
		Ok(_) => assert!(false, "should be underpriced"),
		Err(e) => match e {
			InsertErr::FeeCapBelowMinimumProtocolFeeCap { .. } => {
				assert!(true);
			},
			_ => assert!(false, "should be underpriced"),
		},
	}
}

#[test]
fn test_insert_with_nonce_gap() {
	use crate::testutils::{MockTransaction, MockTransactionFactory};
	...
	assert!(replaced_tx.is_none());
	//assert!(state.contains(TxState::ENOUGH_BALANCE));
	assert_eq!(move_to, SubPool::Parked);
	let inserted = pool.txs.get(&valid_tx.transaction_id).unwrap();
	assert_eq!(inserted.subpool, SubPool::Parked);
}

#[test]
fn test_mempool_default_initialization_and_add_transaction() {
	use crate::testutils::{MockTransaction, MockTransactionFactory};

	let mut mempool: Mempool<FeeOrdering<MockTransaction>> = Mempool::default();
	...
	let mut tx_1 = MockTransaction::default();
	tx_1.set_gas_price(MIN_PROTOCOL_BASE_FEE + 1000); // Set gas price above minimum
	let valid_tx_1 = factory.validated(tx_1);
	...
	let mut inserted_result =
		mempool.add_transaction(valid_tx_1.clone(), on_chain_balance, on_chain_nonce);

	// Verify successful insertion
	assert!(inserted_result.is_ok());

	let inserted_tx = inserted_result.unwrap();

	match inserted_tx {
		AddedTransaction::Pending(added_pending_tx) => {
			assert_eq!(added_pending_tx.transaction.hash(), valid_tx_1.hash());
			assert!(added_pending_tx.replaced.is_none());
			assert!(added_pending_tx.discarded.is_empty());
			assert!(added_pending_tx.promoted.is_empty());
		},
		_ => {
	...
	let do_exist = mempool.pending_pool.contains(&valid_tx_1.transaction_id);
	assert!(do_exist);

	let mut tx_2 = MockTransaction::default();
	tx_2.set_gas_price(MIN_PROTOCOL_BASE_FEE + 1000); // Set gas price above minimum
	tx_2.set_hash(TxHash::new([2; 32]));
	// set nonce to 2
	tx_2.inc_nonce().inc_nonce();
	let valid_tx_2 = factory.validated(tx_2);

	// Insert the transaction into the mempool
	let mut inserted_result =
		mempool.add_transaction(valid_tx_2.clone(), on_chain_balance, on_chain_nonce);
	...
	match inserted_tx {
		AddedTransaction::Pending(..) => assert!(false, "should be parked"),
		AddedTransaction::Parked { transaction, replaced, .. } => {
			assert_eq!(transaction.hash(), valid_tx_2.hash());
			assert!(replaced.is_none());
		},
	}

	let do_exist = mempool.parked_pool.contains(&valid_tx_2.transaction_id);
	assert!(do_exist);

	// Create a valid transaction
	let mut tx_3 = MockTransaction::default();
	// Set gas price above minimum
	tx_3.set_gas_price(MIN_PROTOCOL_BASE_FEE + 1000);
	tx_3.set_hash(TxHash::new([3; 32]));

	// set nonce to 1
	tx_3.inc_nonce();

	let valid_tx_3 = factory.validated(tx_3);
	...
	// Insert the transaction into the mempool
	let mut inserted_result =
		mempool.add_transaction(valid_tx_3.clone(), on_chain_balance, on_chain_nonce);
	...
	match inserted_tx {
		AddedTransaction::Pending(added_pending_tx) => {
			assert_eq!(added_pending_tx.transaction.hash(), valid_tx_3.hash());
			assert!(added_pending_tx.replaced.is_none());
			assert!(added_pending_tx.discarded.is_empty());
		},
		_ => {
	...
	// let InsertOk { transaction, move_to, state, replaced_tx, updates } = insert_result.unwrap();

	// // Verify transaction details
	// assert_eq!(transaction.hash(), valid_tx.hash());
	// assert_eq!(move_to, SubPool::Pending);
	// assert!(state.contains(TxState::NO_NONCE_GAPS));
	// assert!(state.contains(TxState::ENOUGH_BALANCE));
	...
	// // Verify mempool state after insertion
	// assert_eq!(mempool.len(), 1);
	// assert!(!mempool.is_empty());
	// assert!(mempool.contains(valid_tx.hash()));

	// // Verify transaction is in the correct state
	// let inserted_tx = mempool.get(&valid_tx.transaction_id).unwrap();
	// assert_eq!(inserted_tx.subpool, SubPool::Pending);
	// assert_eq!(inserted_tx.transaction.hash(), valid_tx.hash());
	// assert_eq!(inserted_tx.cumulative_cost, 0); // First transaction has no cumulative cost

	// // Verify transaction counter
	// assert_eq!(mempool.tx_count(valid_tx.sender_id()), 1);

	// // Verify invariants
	// mempool.assert_invariants();
}
...
// 			PoolConfig { queued_limit, ..Default::default() },
// 		);
...
// 		let mut pool = TxPool::new(
// 			MockOrdering::default(),
// 			PoolConfig { blob_limit, ..Default::default() },
// 		);
