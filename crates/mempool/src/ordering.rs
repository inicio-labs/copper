...
pub trait TransactionOrdering: Debug + Default + Send + Sync + 'static {
...
///
/// The transactions are ordered by their fees
/// The higher the fees is, the higher the priority of the transaction.
#[derive(Debug)]
#[non_exhaustive]
pub struct FeeOrdering<T>(PhantomData<T>);

impl<T> TransactionOrdering for FeeOrdering<T>
where
	T: PoolTransaction + 'static,
{
	...
	fn priority(&self, transaction: &Self::Transaction) -> Priority<Self::PriorityValue> {
		Priority::Value(transaction.cost())
	}
}

impl<T> Default for FeeOrdering<T> {
	fn default() -> Self {
		Self(Default::default())
	}
}

impl<T> Clone for FeeOrdering<T> {
	fn clone(&self) -> Self {
		Self::default()
	}
}
